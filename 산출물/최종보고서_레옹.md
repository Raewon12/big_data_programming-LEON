...
|---|---|---|---|---|---|---|---|
| 팀장 | 정래원 | 2071117 | 한성대학교 | 010-2205-6501 | fodnjs15@hansung.ac.kr | 빅데이터트랙 | 모바일소프트웨어트랙 |
| 팀원 | 김동현 | 2371096 | 한성대학교 | 010-6290-1528 | 0424dhdh@hansung.ac.kr | 모바일소프트웨어트랙 | 빅데이터트랙 |
| 팀원 | 김홍근 | 2191049 | 한성대학교 | 010-9859-3459 | khg9859@gmail.com | 웹공학트랙 | 빅데이터트랙 |
| 팀원 | 이승준 | 1971090 | 한성대학교 | 010-2207-8646 | netisky2@hansung.ac.kr | 빅데이터트랙 | 웹공학트랙 |
| 팀원 | 이윤수 | 2171120 | 한성대학교 | 010-8277-4058 | mooksu2@daum.net | 모바일소프트웨어트랙 | 빅데이터트랙 |
...
| 5 | Late Fusion (α=0.8) | 앙상블 | 0.7073 | **0.6561** | ▲ 최고 |
...
**[증빙사진 1] EDA — 클래스 분포 분석**

![Train 데이터 클래스 분포](../EDA/class_distribution.png)
...
**[증빙사진 3] RunPod GPU 환경 BERT+MLP 학습**

![BERT+MLP 학습 로그](../report_images/bert_mlp_epochs.png)

*그림 3. RunPod GPU(NVIDIA RTX PRO 4500) 환경 KLUE-BERT 임베딩 기반 MLP 학습 (early stopping, best val F1 0.6003)*

---

**[증빙사진 4] Late Fusion α 탐색**

![Late Fusion α 탐색](../artifacts/late_fusion_alpha.png)

*그림 4. Late Fusion α vs Validation Macro F1 (최적 α=0.8, val F1 0.6669)*

---

**[증빙사진 5] BERT+MLP 혼동 행렬 (test 4,261건)**

![BERT+MLP 혼동 행렬](../artifacts/confusion_matrix_bert_mlp.png)
...
결측 제거 **전후 클래스 분포 비교**는 당시 전처리 노트북에서 수행하였으나, 제거 전 중간 CSV가 저장소에 남아 있지 않아 **클래스별 제거 비율의 정량 재검증은 불가**하다. 다만 최종 42,601건 train 분포에서 10개 클래스가 모두 존재하고, stratify 분할 후에도 소수 클래스(개인정보/ICT)가 val·test에 포함되어 있어 **특정 클래스가 전면 소실된 편향은 관찰되지 않았다**. 정밀 검증은 `preprocess/make_processed_data.py`로 원본 JSON부터 재생성 시 전후 분포를 출력하도록 보완하였다.
...
*그림 6. 10개 사건 유형별 Train 데이터 분포*

**발견 2 — 입력 정보밀도 차이**

![텍스트 길이 분포](../EDA/text_length_distribution.png)

*그림 7. 판시사항·요약문·결합 텍스트 길이 분포*

**발견 3 — 유사 카테고리 존재**

![유사도 히트맵](../EDA/category_similarity_heatmap.png)

*그림 8. 카테고리 간 TF-IDF 코사인 유사도 히트맵*

**발견 4 — 도메인 키워드 변별력**

![대표 키워드](../EDA/class_keywords.png)
...
![베이스라인 혼동 행렬](../baseline/confusion_matrix_baseline.png)

*그림 10. 베이스라인 TF-IDF+SVM 혼동 행렬 (튜닝 후, test 4,261건)*
...
14. Lee, J. Legal text classification in Korean sexual offense cases: from traditional machine learning to large language models with XAI insights. *Artificial Intelligence and Law*. **[참고문헌 정보 확인 필요 — 저자 전명·연도·권호·DOI/URL 미확인, 임의 보완 금지]**
...
> ⚠️ **팀 직접 삽입 필수** — 아래 2장은 저장소에 포함되지 않습니다. Word 제출본에 실제 사진을 붙여 넣으세요. 확인 항목은 `산출물/제출전_체크리스트.md`를 참고하세요.

**[첨부사진 1 — 팀 단체 사진]**

*(Word 제출 시 여기에 팀원 전체 단체 사진 삽입)*

*캡션: 팀 레옹 단체 사진 (2026학년도 1학기 빅데이터프로그래밍 프로젝트)*

---

**[첨부사진 2 — 프로젝트 수행 활동]**

*(Word 제출 시 여기에 미팅·실험·최종 발표·Q&A 등 활동 사진 삽입)*

*캡션: 최종 발표 Q&A 세션 또는 프로젝트 수행 활동 사진*
